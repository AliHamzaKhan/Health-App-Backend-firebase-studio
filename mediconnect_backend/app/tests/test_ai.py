    ...
    def test_report_analysis(self, mock_gemini):
        ...
        response = client.post("/api/v1/ai/report-analysis", json={"report": "This is a test report."})
        self.assertEqual(response.status_code, 200)
    ...
    def test_symptom_checker(self, mock_gemini):
        ...
        response = client.post("/api/v1/ai/symptom-checker", json={"symptoms": ["cough", "sore throat"]})
        self.assertEqual(response.status_code, 200)
    ...
    def test_allergy_checker(self, mock_gemini):
        ...
        response = client.post("/api/v1/ai/allergy-checker", json={"symptoms": ["runny nose", "sneezing"], "medical_history": ["hay fever"]})
        self.assertEqual(response.status_code, 200)
    ...
    def test_calorie_checker(self, mock_gemini):
        ...
        response = client.post("/api/v1/ai/calorie-checker", json={"meal_description": "two slices of bread"})
        self.assertEqual(response.status_code, 200)
